...
for i, ins in enumerate(inscriptions):
    ...
    js_parts.append(f'nodes.add({{id:"i_{nid}",label:"#{nid}",group:"inscription",title:"{name}",region:"{region}",period:"{period}",class:"{cls}",sect:"{sect}",deity:"{deity}",patron:"{patron}",shape:"dot",size:5}});')
...
print(f"  Regions: {len(regions)}, Periods: {len(periods)}, Classes: {len(classes)}, Sects: {len(sects)}")
print(f"  Inscriptions: {len(inscriptions)}")
print(f"  Edges: {len(inscriptions) * 4}")
